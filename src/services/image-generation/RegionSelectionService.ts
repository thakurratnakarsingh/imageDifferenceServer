...
import { AnalysisRegion, DifferenceRegion, ModificationType } from './types';
import { env } from '../../config/env';

const types: ModificationType[] = [
...
function overlaps(a: DifferenceRegion, b: DifferenceRegion) {
  const gap = env.MIN_DIFFERENCE_GAP;
...
export class RegionSelectionService {
  select(candidates: AnalysisRegion[], width: number, height: number, difficulty: keyof typeof sizeByDifficulty, seed: string) {
    const chosen: DifferenceRegion[] = [];
    ...
    for (let index = 0; index < 10; index += 1) {
      ...
      for (let attempt = 0; attempt < ranked.length && !completed; attempt += 1) {
        ...
        if (!chosen.some(other => overlaps(region, other))) { chosen.push(region); completed = true; }
      }
      if (!completed) throw new Error('Unable to select 10 non-overlapping safe regions');
    }
    return chosen;
  }
}

export const regionsOverlap = overlaps;
